...
def meta_scraper(table):
    months = ['2017/11','2017/10','2017/09','2017/08','2017/07','2017/06','2017/05',
                '2017/04','2017/03','2017/02','2017/01','2016/12','2016/11']



    archives = ['http://www.politicususa.com/{}/'.format(month) for month in months]

    for archive in archives:
        soup = souper(archive, False)
        next_pages = soup.find_all('a',{'class':'page-numbers'})
        hits = int(next_pages[-2].get_text())
        p_ns = np.arange(1,hits+1)
        links = [archive+'page/'+n for n in p_ns]
        for link in links:
...
if __name__ == '__main__':
    table = open_database_collection('pu')
    #meta_scraper(table)
